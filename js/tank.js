$(function () {
    ...
    for (i = 1; i <= 50; i++) {
        ...
        let fillOutDate = randomDate(oldDate, todayDate);
        let company = (Math.random() >= 0.5) ? '流亞' : '亞磯';
        let process = (Math.random() >= 0.5) ? 1 : 4;

        let startDate = getStartDate(new Date(fillOutDate));
        let endDate = getEndDate(new Date(fillOutDate));
        let appointmentDate = startDate + '~' + endDate;

        jsonArray.push({
            'id': i,
            'seriesNo': seriesNo,
            'fillOutDate': getMyDate(fillOutDate),
            'company': company,
            'process': process,
